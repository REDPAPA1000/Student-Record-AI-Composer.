...
### [핵심 목표: 분량 확보 및 개별화]
- **분량 최우선 규칙**: 반드시 **공백 포함 최소 1400바이트 ~ 1500바이트(한글 기준 약 480자 ~ 510자)**를 꽉 채워 작성하십시오. 
- 만약 입력된 관찰 내용이 짧다면, 해당 키워드와 관련된 학교 생활 에피소드(수업 태도, 모둠 활동, 학교 행사 등)를 **상상하여 논리적이고 구체적으로 덧붙여서** 분량을 충분히 확보하십시오. 절대로 짧게 끝내지 마십시오.
- 강조점: **"${randomVariance}"**

### [절대 규칙]
1. **작성 형식**: 주어(학생은, 위 학생은)는 생략하고, 문장은 반드시 **"~함."** 또는 **"~임."** 등의 명사형 어미와 **마침표(.)**로 끝나야 합니다.
2. **문단 구조**: START 기법(상황-역할-행동-결과-성장)을 사용하여 하나의 완성된 문단으로 작성하십시오.
3. **스타일**: ${tone}
4. 마지막 문장은 반드시 학생의 **발전 가능성**이나 **교사의 기대감**으로 마무리하십시오.
